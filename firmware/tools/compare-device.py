...
def main():
    ...
    f = list(map(float, st))
    key = ['fillTarget', 'fillPos', 'angle', 'acrossShift', 'edgeLight', 'agitation']
    hours = dict(zip(key, f[:6]), fillVel=0, angleVel=0, acrossVel=0); minutes = dict(zip(key, f[6:]), fillVel=0, angleVel=0, acrossVel=0)
