...
`C_i(u) = E_l · [ R_i(d, Rg_i) + F_far(u) · Tr_i(d)² ] + C_side,i(u)`,
`Rg_i = lin(back_i(t)) · E_b / E_l` (0 when `E_l` = 0). The first term is the backing seen through the
...
| `bubbleRim` | `enc255(0.35·C(0) + 0.65·white)`; `bubbleDark = 0.25 + 0.5·(1 − T)`. |
...
| oil on parchment | olive oil above with back #f1e6cf | medium / opaque, T_lum 0.47 but T_max 0.063 → T 0.063 | liquid (116,101,0), liquidLo (39,32,0), liquidHi (220,210,163), residual 0 (the light backing is carried by the body, not by the mix) |
| pinot on parchment | μ 1.5, ρ 990, γ 46, ior 1.35, K (0.15, 0.65, 0.5), θ 20 ± 10, solids 0.03, back #f1e6cf | watery / opaque, T_lum 0.07, T_max 0.012 → 0.012 | liquid (79,0,6), liquidLo (29,0,0), liquidHi (231,166,172), residual 0; colour and class only | freeDamp 0.89, bounce 0.18, K 418, damp 8.9, contactDyn 9.3 | wetFilm 11.8, fizz 1.35 px / 38 px/s / 42 |
