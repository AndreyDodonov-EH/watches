// The physical signature collection: one material preset per legacy signature look. Each entry is a
// material (values decided in the plan, handbook where the liquid is known) plus a design (the
// allowlisted legacy keys the legacy preset of the same id sets explicitly in params.ts: layout,
// backing, marks, digits). derive() fills every other Params key. Every entry must derive without
...
import { DEFAULT_PARAMS, PRESETS } from '../params';
...
function designOf(id: string, extra: Design = {}): Design {
  ...
  for (const k of DESIGN_KEYS) if (Object.prototype.hasOwnProperty.call(e.p, k)) d[k] = e.p[k];
  const out: Design = { ...(d as Design), ...extra };
...
export const MATERIAL_PRESETS: readonly MaterialPreset[] = [
  ...
  entry('honey', 'honey: amber syrup, viscous, clings to the glass, trapped air', 'handbook (viscosity 20 °C ~10 Pa·s); absorption estimated',
    { viscosity: 10000, density: 1420, surfaceTension: 70, ior: 1.49, absorptionR: 0.06, absorptionG: 0.18, absorptionB: 0.7, scattering: 0, contactAngle: 25, contactHysteresis: 20, solidsFraction: 0.8, dryingTime: 2, gasMode: 3, gasLevel: 0.4, bubbleRadius: 0.12, foamStability: 20, exposure: 1.6 },
    { handbook: true }),
  ...
  entry('cola', 'cola: dark translucent brown, watery, lively bead, enamel numerals', 'estimated',
    { viscosity: 1.2, density: 1040, surfaceTension: 60, ior: 1.35, absorptionR: 0.07, absorptionG: 0.25, absorptionB: 0.7, scattering: 0, contactAngle: 20, contactHysteresis: 10, solidsFraction: 0, dryingTime: 1, gasMode: 1, gasLevel: 0.5, bubbleRadius: 0.07, foamStability: 4, exposure: 1.3 },
    {}),
  ...
  entry('molten', 'molten iron: emissive orange liquid metal, dense, non-wetting, forged numerals', 'handbook (1600 °C); emission artistic',
    { viscosity: 6, density: 7000, surfaceTension: 1800, ior: 1, metallic: 1, metalReflectance: 0.3, emissionR: 0.9, emissionG: 0.3, emissionB: 0.03, contactAngle: 120, contactHysteresis: 10, solidsFraction: 0, gasMode: 3, gasLevel: 0.5, bubbleRadius: 0.1, foamStability: 15 },
    { handbook: true, over: { ior: 'estimated' } }),
  ...
  entry('spritz', 'Aperol spritz on white: vivid orange aperitivo, lively bead, navy enamel numerals', 'estimated',
    { viscosity: 1.3, density: 1000, surfaceTension: 50, ior: 1.35, absorptionR: 0.04, absorptionG: 0.5, absorptionB: 1.2, scattering: 0, contactAngle: 20, contactHysteresis: 10, solidsFraction: 0, dryingTime: 1, gasMode: 1, gasLevel: 0.6, bubbleRadius: 0.08, foamStability: 2.5 },
    // design adjusted: legacy tickBright 2 / digitBright 2 (rejection 11)
